...
namespace Netgusto\Baikal\FrontendBundle\Service;
...
class AccessDeniedHandler implements AccessDeniedHandlerInterface {
    ...
    public function handle(Request $request, AccessDeniedException $accessDeniedException) {
        ...
        if($this->securityContext->isGranted('ROLE_ADMIN') || $this->securityContext->isGranted('ROLE_STATIC_ADMIN')) {
            return new RedirectResponse($this->router->generate('netgusto_baikal_admin_homepage'));
        }

        # If not, we display an "Access denied" message
        $attributes = array(
            '_controller' => 'NetgustoBaikalCoreBundle:Security:accessDenied',
